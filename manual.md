...
10. [附录：四端功能矩阵](#10-附录四端功能矩阵)
11. [技术规格与环境](#11-技术规格与环境)
12. [安全与合规](#12-安全与合规)
13. [常见问题（FAQ）](#13-常见问题faq)
14. [版权声明与证明文件](#14-版权声明与证明文件)
...
| 模式 | 入口 | 用途 |
| --- | --- | --- |
| 普通打卡模式 | AgoraInPro「大课堂模式」 | 单教室/单设备本地打卡：点名矩阵、排名、CSV 名单 |
| 集控打卡模式 | AgoraInPro「控制模式」（控制中心）+ 服务器/移动端 | 多设备集中管理：任务下发、呼叫、课表、数据同步 |
| 扫码签到 | 任意端创建任务 → 学生扫二维码 → 手机网页签到 | 无接触签到，生成短链接/二维码，支持签到密码与防重复 |
| 呼叫联动 | 服务器/移动端/客户端 → ClassIsland 插件 | 应急/传唤/下课提醒，课表感知 + 语音播报 + 倒计时提示栏 |
...
大课堂模式适用于单教室的单机打卡，是「一台客户端 + 一台屏幕」的经典使用方式：

1. **创建签到任务**：「远程 → 创建签到」（或点左侧任务树 `+`），填写签到密码、确认密码、教室、科目，并可「导入 CSV」直接生成学生名单（也可使用打卡功能导出的 CSV）；
2. **学生打卡**：将客户端窗口投影到教室大屏/平板，学生点击矩阵中自己的名字即完成打点（已打卡学生高亮），右键已打卡学生可取消打卡；
3. **实时反馈**：
   - 右侧打卡排名实时刷新（打卡时间、最早打卡标记）；
   - 右上角显示「总人数: N | 已打卡: M (X%)」统计；
   - 底部状态栏显示「服务器: 在线」。
4. **任务管理**：左侧任务树（我的任务 → 任务列表）支持切换多个任务窗口（如「默认任务 - 数学」），`+` 新建、✕ 关闭标签页。

### 6.5 集控打卡模式（控制中心）

切换「控制模式」后进入控制中心，把设备、任务、课表与呼叫集中在一个界面管理：

- **集控平台列表**：显示平台连接状态（已自动连接 N 个集控平台）；
- **任务中心**：查看/管理平台上的全部签到任务；
- **设备列表（呼叫面板）**：勾选目标设备 →「发送到所选」可向指定设备远程发送呼叫或任务；每行「呼叫」按钮支持单台快速呼叫；「刷新设备」拉取最新在线状态；
- **课时划消与排课（课时统计）**：
  - 左侧为学生列表：为学生设置**初始课时数**（支持 1 位小数，如 1.5）、自动生成「总 X 课时 / 已划 Y / 剩余 Z」课时统计；
  - 右侧日历排课：按日期选择（今天/翻月），支持「设为不排课日/恢复」「复制排课」「清空排课」；
  - 每名学生可设置「上课/下课时间」，点击学生添加到日历形成**提醒排课**，到点后自动触发对应的上下课提醒；
  - 已排课学生显示在「已排课学生（可修改时间/移除）」区域，支持「忘记课、请假调课」；
- **数据同步**：「远程 → 从服务器加载数据 / 同步数据到服务器」将本机任务与打卡数据与集控平台双向同步，多台设备共享同一份数据。

### 6.6 扫码签到（学生端）

教师端创建签到任务后将获得一个随机短链（如 `/s/a1B2c3`）与二维码：

1. 客户端「创建签到」（或移动端「二维码」页）生成任务，服务器分配短链码并生成二维码图片（可保存/打印）；
2. 学生用手机**扫描二维码**（或手动输入短链地址）打开签到网页；
3. 在网页中输入姓名与签到密码 → 提交，即完成打卡；同一设备重复签到会被拒绝（「您已签到成功，无需重复签到」）；
4. 签到记录实时汇聚到服务器（姓名 + 时间），教师端/Web 端可查看该任务的签到名单与打卡进度。

> 签到任务字段：短链码（唯一索引）、签到密码、教室、科目、任务名称、学生名单（JSON 数组）、签到记录（[{name,time}]）、状态（进行中/已关闭）。
...
- **任务列表与详情**：展示任务名称、状态（进行中/已关闭）、签到进度（如 3/40）、创建时间；点击任务进入详情查看已签到学生与签到时间；
- **管理员任务管理**（管理员角色，经由「任务管理」进入）：
  - 重命名任务（如将默认任务改名）；
  - 删除任务（含确认提示）；
  - 查看任务考勤详情（科目、状态、签到进度、已签到名单）；
- **考勤汇总**：「我的」页面展示总签到次数、任务数与各任务「已签到: 学生5、学生8、学生13」明细。

![移动端任务列表](/userguide/mobile-tasks.png)

### 8.4 二维码（生成签到码 / 扫码签到）

- 创建签到任务：选择设备、科目、教室、签到密码、学生名单（每行一个姓名，或提示 CSV 导入）；
- 生成后保存/展示**签到二维码与短链**，学生扫码即打开签到网页（输入姓名 + 签到密码 → 打卡成功）；
- 任务创建后同步到服务器，教师端与 Web 端随时可查看签到进度。
...
1. 教师端（客户端/移动端）「创建签到」→ 生成二维码并投放到屏幕/教室一体机；
2. 学生扫二维码 → `签到网页` 填写姓名与签到密码 → 提交；
3. 服务器保存签到记录（姓名+时间，防重复签到），教师端打卡矩阵/移动端「我的 → 签到进度」实时联动更新；
4. 班级 40 人名单支持 CSV 导入，签到进度以 「已签到: 学生5、学生8、学生13」等明细展示。
...
| 功能 | 桌面客户端 AgoraInPro | 服务器 Web 面板 | 移动端 App | ClassIsland 插件 |
| --- | --- | --- | --- | --- |
| 普通打卡模式（点名矩阵/排名/CSV） | ✅ 核心 | — | 查看进度 | — |
| 创建签到任务 | ✅（创建签到/CSV 导入） | 设备详情只读 | ✅（二维码页/任务管理） | — |
| 扫码签到（短链+密码） | 生成 | 短链网页渲染（`/s/{code}`） | 生成/保存二维码 | — |
| 集控打卡（多设备任务下发与同步） | ✅ 控制中心 | 设备/任务管理 | ✅ 控制页 | — |
| 任务管理（重命名/删除/进度） | ✅ 任务中心 | ✅ 设备详情任务列表 | ✅ 管理员任务管理 | — |
| 呼叫发起（三种类型） | ✅ 设备列表→发送到所选 | ✅ /calls | ✅ 控制页 | — |
| 呼叫接收/播报 | — | — | — | ✅ 提示栏+倒计时+TTS |
| 课表感知呼出（上课暂缓/下课呼出） | — | — | — | ✅ 依据 ClassIsland 课表 |
| 课时统计（总/已划/剩余/排课日历） | ✅ 控制中心课时划消与排课 | — | 展示统计 | — |
| 设备管理（在线状态/重命名/删除） | ✅ 控制中心设备列表 | ✅ 设备总览/详情 | ✅ 设备列表 | 自登记 |
| 用户管理 | — | ✅ /users | ✅ 用户页 | — |
| 系统日志/审计 | — | ✅ /logs | — | — |
| 修改密码/协议密码 | ✅ 远程服务器设置 | ✅ /profile | — | 配置页 |

> 说明：插件专注「呼叫输出端」——不承担任务管理；任务创建、下发与查看由客户端/Web/移动端完成。
