...
> 本文档对应 **AgoraIn v3.2**（AgoraInPro）。历史版本：v2.8 已归档（[v2.8 功能特性](/v2.8/features)），v2.7 已归档（[v2.7 功能特性](/v2.7/features)。

AgoraIn v3.2 在一个窗口内提供**大屏模式**与**控制模式**两种工作模式，配套集控服务器与移动端 App。
...
- **初始化设置向导**：首次部署自动跳转到向导页面（`/setup`），引导完成服务器名称、管理员密码等基础配置，无需手动编辑配置文件
- **服务器离线激活**：通过硬件指纹绑定授权（`/api/server/fingerprint` + `/api/server/activate`），支持无网络环境下完成授权验证，适配内网部署与安全合规场景
...
- **呼叫通知**：`/api/mobile/calls` 发送呼叫，设备端 `/api/calls_pull` 拉取、`/api/calls_ack` 确认；三种类型（待下课/应急/传唤），自动过期标记
...
## ClassIsland 联动插件（AgoraIn.ClassIslandPlugin）

- 运行于 **ClassIsland 2.x** 主程序进程内（`ClassIsland.Plugin.cipx` 安装包，2.1.1.0）
- 定时轮询集控平台（默认 10 秒，可配置），拉取本机待处理呼叫
- **待下课时段通知**：缓存并结合 ClassIsland 课表，在"距下课 ≤ 提前分钟数"窗口自动提醒
- **上课应急通知 / 下课传唤**：立即弹出置顶窗口（红 / 橙主题色），并**中文语音朗读**（System.Speech，自动选择中文语音，同一呼叫不重复朗读）
- 自动探测本机 AgoraIn 大屏客户端配置（`Program Files` / 本地程序目录 / `AGORAIN_HOME`）
- 配置持久化到 ClassIsland 插件目录 `settings.json`（平台地址 / 密码 / 设备 UUID 必须与教师端发送目标一致）
...
**新增**
- 控制模式（控制中心）：统计条、左侧导航、划课页、设备列表（呼叫面板）、任务中心、集控平台列表
- 呼叫通知：教师端批量/单台发送呼叫（待下课 / 应急 / 传唤），设备弹窗 + 语音朗读
- ClassIsland 联动插件 2.1.1.0：轮询修复、中文语音朗读
- 平台配置持久化（platforms.json，密码 DPAPI 加密），重启自动重连
